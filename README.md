...
## Overview
This project implements a cloud resume using a modern, serverless AWS architecture. The goal is not only to host a personal resume, but to design and operate a secure, scalable, and cost-efficient web application using cloud services.

The solution uses Amazon S3 + CloudFront for global static content delivery, a serverless backend (API Gateway, Lambda, DynamoDB) for interactive visitor tracking, and Terraform to provision all infrastructure. Configuration management and deployment orchestration are handled with Ansible, ensuring repeatable, environment agnostic deployments.

The project is designed to mirror real-world architectural decision making, while considering cost, scalability, and security.
...
1. User Requests The Website
    - Traffic is routed to CloudFront, which uses an AWS-managed certificate to securely handle HTTPS.
...
Each major subsystem (frontend, backend content pipeline, cloud infrastructure) is documented in its own README for clarity and separation of concerns. Navigate to each below:
